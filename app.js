...
function sumAndMultiply(a, b, c) {
  ...
  let multResult = multiply(a, multiply(b, c)[0])[0];
  return [sumResult, multResult, `${a} and ${b} and ${c} sum to ${sumResult}.`, `The product of ${a} and ${b} and ${c} is ${multResult}.`];
...
function sumArray(sumArr) {
  ...
  for (let i = 0; i < sumArr.length; i++) {
    sumArrRes = sum(sumArrRes, sumArr[i])[0];
  }
  return [sumArrRes, `${sumArr} was passed in as an array of numbers, and ${sumArrRes} is their sum.`];
}
...
function multiplyArray(multArr) {
  ...
  for (let i = 0; i < multArr.length; i++) {
    multArrRes = multiply(multArrRes, multArr[i])[0];
  }
  return [multArrRes, `The numbers ${multArr} have a product of ${multArrRes}.`];
}
